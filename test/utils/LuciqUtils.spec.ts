...
describe('Test global error handler', () => {
  beforeEach(() => {
    Luciq.init({ token: '', invocationEvents: [InvocationEvent.none] });
  });
...
describe('Luciq Utils', () => {
  ...
  it('getActiveRouteName should get route name from navigation state', () => {
    ...
    // @ts-ignore
    const currentScreen = LuciqUtils.getActiveRouteName(navigationState);
  ...
  it('getActiveRouteName should return null if no navigation state', () => {
    // @ts-ignore
    const output = LuciqUtils.getActiveRouteName(null);
  ...
  it('getFullRoute should get route name from navigation state', () => {
    ...
    const currentScreen = LuciqUtils.getFullRoute(navigationState);
  ...
  it('getFullRoute should return an empty string if navigation state is invalid', () => {
    // @ts-ignore
    const output = LuciqUtils.getFullRoute({});
    expect(output).toBe('');
  });
  ...
  it("parseErrorStack should call React Native's parseErrorStackLib", () => {
    ...
    LuciqUtils.parseErrorStack(error);
  ...
  it('getStackTrace should call parseErrorStackLib with error stack in React Native >= 0.64', () => {
    ...
    const frames = LuciqUtils.getStackTrace(error);
  ...
  it('stringifyIfNotString should stringify non-strings', () => {
    expect(LuciqUtils.stringifyIfNotString('hello')).toBe('hello');
    expect(LuciqUtils.stringifyIfNotString(100)).toBe('100');
    expect(LuciqUtils.stringifyIfNotString([])).toBe('[]');
  });
  ...
  it('should call remoteSenderCallback with the correct JSON object on Android', () => {
    ...
    const jsStackTrace = LuciqUtils.getStackTrace(errorMock);

    LuciqUtils.sendCrashReport(errorMock, (data) =>
  ...
  it('getCrashDataFromError should include one level of cause crash', () => {
    ...
    const crashData = LuciqUtils.getCrashDataFromError(rootError);
    const jsStackTraceRootError = LuciqUtils.getStackTrace(rootError);
    const jsStackTraceCauseError = LuciqUtils.getStackTrace(causeError);
...
describe('reportNetworkLog', () => {
  ...
  it('reportNetworkLog should send network logs to native with the correct parameters on Android', async () => {
    ...
    LuciqUtils.reportNetworkLog(network);

    expect(NativeLuciq.networkLogAndroid).toHaveBeenCalledTimes(1);
    expect(NativeLuciq.networkLogAndroid).toHaveBeenCalledWith(
      network.url,
      network.requestBody,
  ...
  it('reportNetworkLog should send network logs to native with the correct parameters on iOS', () => {
    ...
    expect(NativeLuciq.networkLogIOS).toHaveBeenCalledTimes(1);
    expect(NativeLuciq.networkLogIOS).toHaveBeenCalledWith(
      network.url,
      network.method,
      network.requestBody,
      network.requestBodySize,
...
describe('test registerNetworkLogsListener usage', () => {
  ...
  it('registerFilteringListener should call NetworkLogger.registerNetworkLogsListener() with  NetworkListenerType = NetworkListenerType.filtering', () => {
    const testText = 'true';
    LuciqUtils.registerFilteringListener(testText);
  ...
  it('registerFilteringAndObfuscationListener should call NetworkLogger.registerNetworkLogsListener() with  NetworkListenerType = NetworkListenerType.both', () => {
    const testText = 'true';
    LuciqUtils.registerFilteringAndObfuscationListener(testText);
  ...
  it('should call NetworkLoggerEmitter.removeAllListeners when call resetNativeObfuscationListener', () => {
    ...
    LuciqUtils.resetNativeObfuscationListener();
  ...
  it('should call  NativeNetworkLogger.updateNetworkLogSnapshot when call updateNetworkLogSnapshot with correct parameters', () => {
    ...
    LuciqUtils.updateNetworkLogSnapshot(network);
...
describe('LuciqUtils', () => {
  it('setApmNetworkFlagsIfChanged should return true if flags change', () => {
    ...
    expect(LuciqUtils.setApmNetworkFlagsIfChanged(flags)).toBe(true);
    expect(LuciqUtils.setApmNetworkFlagsIfChanged(flags)).toBe(false);
  });

  it('generateTracePartialId should return a non-zero hex string and number', () => {
    const { numberPartilId, hexStringPartialId } = LuciqUtils.generateTracePartialId();
  ...
  it('generateW3CHeader should return a valid w3c header object', () => {
    const now = Date.now();
    const result = LuciqUtils.generateW3CHeader(now);
  ...
  it('isContentTypeNotAllowed should return false for allowed types and true for not allowed', () => {
    expect(LuciqUtils.isContentTypeNotAllowed('application/json')).toBe(false);
    expect(LuciqUtils.isContentTypeNotAllowed('text/plain')).toBe(false);
    expect(LuciqUtils.isContentTypeNotAllowed('image/png')).toBe(true);
    expect(LuciqUtils.isContentTypeNotAllowed('application/pdf')).toBe(true);
  });
});
