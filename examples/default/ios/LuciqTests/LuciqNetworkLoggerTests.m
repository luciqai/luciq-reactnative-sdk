...
@interface LuciqNetworkLoggerBridgeTests : XCTestCase

@property (nonatomic, strong) LuciqNetworkLoggerBridge *networkLoggerBridge;

@end

@implementation LuciqNetworkLoggerBridgeTests

- (void)setUp {
    [super setUp];
    self.networkLoggerBridge = [[LuciqNetworkLoggerBridge alloc] init];
}
...
- (void)testRequiresMainQueueSetup {
    XCTAssertFalse([LuciqNetworkLoggerBridge requiresMainQueueSetup]);
}

- (void)testSupportedEvents {
    NSArray *events = [self.networkLoggerBridge supportedEvents];
    NSArray *expectedEvents = @[@"LCQpreInvocationHandler", @"LCQNetworkLoggerHandler"];
